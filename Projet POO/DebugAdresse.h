...
#pragma endregion
    ...
    void ChargerDonnees()
    {
        String^ connectionString = "Data Source=Martin_GOA;Initial Catalog=Projet;Integrated Security=True"; // Modifiez avec vos informations
        SqlConnection^ connection = gcnew SqlConnection(connectionString);
        SqlCommand^ command = gcnew SqlCommand("SELECT * FROM Adresse", connection); // Assurez-vous que 'Adresse' est le nom correct de votre table
...
private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    {
        int adresseId = Convert::ToInt32(dataGridView1->SelectedRows[0]->Cells["ID_ADRESSE"]->Value); // Assurez-vous que "ID_Adresse" est le nom de la colonne qui contient l'ID
        String^ connectionString = "Data Source=Martin_GOA;Initial Catalog=Projet;Integrated Security=True"; // Modifiez avec vos informations
        SqlConnection^ connection = gcnew SqlConnection(connectionString);
